    ...
    def __init__(self, num_classes=10, path='', is_local=False, diff_classes_flag=False, old_num_classes=10):
        ...
        if self.is_local:
            if self.diff_classes_flag:
                self.backbone = torch.hub.load('pytorch/vision:v0.6.0', 'resnext101_32x8d', pretrained=False)
            ...
            self.backbone = torch.hub.load('pytorch/vision:v0.6.0', 'resnext101_32x8d', pretrained=True)
            replace_classifier(self.backbone, num_classes)
